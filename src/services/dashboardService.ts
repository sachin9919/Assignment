...
export async function getDashboardSummary(params: {
  ...
  const effectiveOwnerId =
    params.viewerRole === "admin" ? params.userId ?? params.viewerUserId : params.viewerUserId;
  ...
  const categoryMap = new Map<
    string,
    { category: string; incomeTotal: number; expenseTotal: number }
  >();
  for (const r of records) {
    const key = r.category;
    const existing = categoryMap.get(key) ?? { category: key, incomeTotal: 0, expenseTotal: 0 };
    if (r.type === "income") existing.incomeTotal += r.amount;
    else existing.expenseTotal += r.amount;
  ...
  if (params.type) trendRecords = trendRecords.filter((r) => r.type === params.type);

  const monthMap = new Map<string, { month: string; incomeTotal: number; expenseTotal: number }>();
